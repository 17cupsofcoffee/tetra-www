...
To add this image to our game, we can use our first new type of the chapter: [`Texture`](https://docs.rs/tetra/0.4/tetra/graphics/struct.Texture.html). This represents a piece of image data that has been loaded into graphics memory.
...
We can then use [`Texture::new`](https://docs.rs/tetra/0.4/tetra/graphics/struct.Texture.html#method.new) to load the sprite and populate that field:
...
To draw our texture, we'll need to make use of another function from the [`graphics`](https://docs.rs/tetra/0.4/tetra/graphics/index.html) module - [`graphics::draw`](https://docs.rs/tetra/0.4/tetra/graphics/fn.draw.html]):
...
If you want to change the rate at which updates happen, or switch to a more traditional 'lockstep' game loop, you can do this via the [`timestep` parameter on `ContextBuilder`](https://docs.rs/tetra/0.4/tetra/struct.ContextBuilder.html#method.timestep).
:::

Inside the `update` method, we can use the functions exposed by the [`input`](https://docs.rs/tetra/0.4/tetra/input/index.html) module in order to check the state of the keyboard:
