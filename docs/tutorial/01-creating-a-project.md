...
Now that we have a `State`, we're ready to start the game loop! To do this, call the [`run`](https://docs.rs/tetra/0.6/tetra/struct.Context.html#method.run) method on `Context`, passing in a closure that constructs your `State` struct:
...
- The return type of `main` has been changed to [`tetra::Result`](https://docs.rs/tetra/0.6/tetra/error/type.Result.html).
